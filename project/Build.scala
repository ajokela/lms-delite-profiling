...
object DeliteBuild extends Build {
  ...
  */
  ...
  lazy val dsls = Project("dsls", file("dsls"), settings = virtBuildSettings) aggregate( optiql, optigraph, profiling) 
  ...
  lazy val apps = Project("apps", file("apps"), settings = virtBuildSettings) aggregate(optiqlApps, optigraphApps)
  ...
  lazy val tests = (Project("tests", file("project/boot"), settings = virtBuildSettings)
    dependsOn( optiqlApps, optigraphApps) aggregate(framework, deliteTest, optiql, optigraph))
